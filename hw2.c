...
int main(int argc, char *argv[]) {   
    ...
    while (j < MAX_BIT_POSITION) {
        cache[j][0] = '0';
        j++;
    }
    // if negative apply rules to convert to 2's complement form
    if (n == 1) {
        for(int k = 0; k < MAX_BIT_POSITION; k++) {
            if (cache[k][0] == '1') {
                cache[k][0] = '0';
            } else if (cache[k][0] == '0') {
                cache[k][0] = '1';
        ...
        int carry = '1';
        int l = 0;
        while(carry != '0' && l < MAX_BIT_POSITION) {
            if (cache[l][0] == '0') {
                cache[l][0] = '1';
                carry = '0';
            } else if (cache[l][0] == '1') {
    ...
    advancedBitwiseFizzBuzz((int32_t)i);
